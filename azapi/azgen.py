...
from .models import AZRequest, AZGPXRequest
...
def get_gpx(request: AZGPXRequest, bounds: Tuple[float, float, float, float]) -> str:
    
    # TODO: Make a function which returns an alphashape to remove redudant code for get_gpx and get_az
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        if os.path.exists(tmpdirname + request.summit_ref + '.gpx'):
            driver.DeleteDataSource(tmpdirname + request.summit_ref + '.gpx')


        out = driver.CreateDataSource(tmpdirname + request.summit_ref + '.gpx')
        ...
        with open(tmpdirname + request.summit_ref + '.gpx') as f:
            output_file = f.read()
